            ...
            {insurance.state >= 6 && <div>
                ...
                {insurance.state >= 0 && insurance.state <= 7 && <CircularProgressbar
                    ...
                    styles={buildStyles({
                        ...
                        pathColor: `rgba(173,216,230, ${(insurance.state / 7) * 1.1 > 1 ? 1 : (insurance.state / 7)})`,
                        textColor: 'lightblue',
                        trailColor: '#212121',
    ...
    const handleSubmitClaimDetails = async (e) => {
        ...
        if (web3) {
            ...
            if (networkData) {
                ...
                // confirm details to be called.
                await deployedContract.methods.SubmitClaimDetails(Number(insurance.ID)-1, crv, crem).send({ from: accounts[0], gas: 3000000 })
                    .then(() => {
                        window.location.reload()
                    })
                // update db
    ...
    const handleSubmitBUWDone = async (e) => {
        ...
        if (web3) {
            ...
            if (networkData) {
                ...
                // confirm details to be called.
                await deployedContract.methods.UpdateFinancialHealth(Number(insurance.ID)-1, cv, act, fhp, buwr).send({ from: accounts[0], gas : 3000000 })
                    .then(() => {
                        window.location.reload()
                    })
                // update db
    ...
    const handleSubmitMWReject = async (e) => {
        ...
        if (web3) {
            ...
            if (networkData) {
                ...
                console.log(deployedContract, 'to change state', accounts[0], insurance.ID, 'also')
                // confirm details to be called.
                await deployedContract.methods.Reject(Number(insurance.ID)-1).send({ from: accounts[0], gas : 3000000 })
                    .then(() => {
                        window.location.reload()
                    })
                insurance.state = -3
    ...
    const handleSubmitMUWDone = async (e) => {
        ...
        if (web3) {
            ...
            if (networkData) {
                ...
                // confirm details to be called.
                await deployedContract.methods.UpdateMedicalHealth(Number(insurance.ID)-1, act, mhp, pv, muwr).send({ from: accounts[0], gas : 3000000 })
                    .then(() => {
                        window.location.reload()
                    })
                // update db
    ...
    const handleSubmitRecalculatePolicy = async (e) => {
        ...
        if (web3) {
            ...
            if (networkData) {
                ...
                // confirm details to be called.
                if (pi===true) {
                await deployedContract.methods.RecalculatePolicy(Number(insurance.ID)-1, pr, pn, issd, matd, pf, saf, pi).send({ from: accounts[0], gas : 3000000 })
                    .then(() => {
                        window.location.reload()
                    })
                // update db
...
const DisplayInsuranceStatus = ({ currentUser, insurance }) => {
    ...
    if (currentUser.user.type==='individual') {
        ...
        return (
            ...
            {insurance.state===1&&<div className="showy">Thanks for confirming your details. The application is being processed. </div>}
            ...
            {(insurance.state===4 || insurance.state === 3)&&<div className="showy">The application is being processed </div>}
        ...
        return (
            <div>
                ...
                {insurance.state===2&&<div className="showy">Requested Bank Underwriting</div>}
                {insurance.state===3&&<RequestMedicalUnderwriting insurance={insurance}/>}
                {insurance.state===4&&<div className="showy">Requested Medical Underwriting</div>}
        ...
        return (
            <div>
                ...
                {insurance.state===3&&<div className="showy">Financial Health Updated. Sent to Insurance Provider. </div>}
                {insurance.state===4&&<div className="showy">Requested Medical Underwriting</div>}
                {insurance.state===5&&<div className="showy">Medical Underwriting Done. Health Score Updated.</div>}
                {insurance.state===6&&<div className="showy">The insurance application was approved. Waiting for client confirmation </div>}
                {insurance.state===7&&<div className="showy">The client has accepted the policy </div>}
                {insurance.state===-1&&<div className="showy">The insurance application was rejected.</div>}
